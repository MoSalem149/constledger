...
async function getProtectedAdminId() {
  ...
  const admin = await User.findOne({ role: "pmo", isActive: true })
    .sort({ createdAt: 1 })
    .select("_id");
...
export const createUser = async (req, res, next) => {
  try {
    ...
    res
      .status(201)
      .json({
        user: { id: user._id, name, email, role, isActive: user.isActive },
      });
  } catch (err) {
    next(err);
  }
...
export const updateUser = async (req, res, next) => {
  try {
    if (await isProtectedAdmin(req.params.id))
      return res
        .status(403)
        .json({ message: "The master PMO account cannot be modified" });
...
export const deleteUser = async (req, res, next) => {
  try {
    if (await isProtectedAdmin(req.params.id))
      return res
        .status(403)
        .json({ message: "The master PMO account cannot be deleted" });
